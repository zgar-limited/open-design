...
async function findCloudSignInButton() {
  return screen.findByRole('button', { name: /Sign in to xDesign/i });
}
...
describe('EntryShell onboarding xDesign AMR runtime', () => {
  it('gates Home on an authoritative signed-out Cloud session without clearing saved setup', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Sign in to xDesign' }),
    ).toBeTruthy();
  ...
  it('shows the model-source chooser after Cloud sign-in without exposing legacy onboarding steps', async () => {
    ...
    expect(screen.getByRole('radio', { name: /xDesign Hosted/i })).toBeTruthy();
  ...
  it('supports arrow-key selection and focus within the model-source radio group', async () => {
    ...
    const hosted = await screen.findByRole('radio', { name: /xDesign Hosted/i });
  ...
  it('does not auto-select xDesign AMR when the AMR runtime is unavailable', async () => {
    globalThis.fetch = vi.fn(async () =>
    ...
    expect(await screen.findByRole('heading', { name: 'Sign in to xDesign' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
    expect(screen.queryByRole('button', { name: /xDesign AMR/i })).toBeNull();
  ...
  it('shows xDesign Cloud as the default connect surface when AMR is available', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({ loggedIn: false, profile: 'prod', user: null, configPath: '/x' }),
    ) as typeof fetch;
    renderOnboarding();

    expect(screen.getByRole('heading', { name: 'Sign in to xDesign' })).toBeTruthy();
    expect(await findCloudSignInButton()).toBeTruthy();
  ...
  it('cancels AMR login and re-enables onboarding after the login timeout', async () => {
    ...
    expect(
      screen
        .getByRole('button', { name: /Sign in to xDesign/i })
  ...
  it('refreshes workspace context, billing, and team projects as soon as onboarding sign-in completes', async () => {
    ...
    // in its signed-out shape (still showing "Sign in to use xDesign
  ...
  it('continues normally when xDesign AMR is signed in', async () => {
    globalThis.fetch = vi.fn(async () =>
      jsonResponse({
        loggedIn: true,
        profile: 'prod',
  ...
  it('keeps the cloud sign-in CTA visible after detection settles without surfacing AMR', async () => {
    ...
    expect(
      await screen.findByRole('button', { name: /Sign in to xDesign/i }),
    ).toBeTruthy();
    expect(screen.queryByRole('button', { name: /xDesign AMR/i })).toBeNull();
    expect(document.querySelector('.onboarding-view__card--skeleton')).toBeNull();
  });
