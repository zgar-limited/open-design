...
type ProjectChatSendMeta = ChatSendMeta & {
  ...
  queueDrain?: boolean;
  /** The xDesign Cloud balance gate already ran for this exact send at
...
function historyWithWorkspaceContext(
  ...
  const block = [
    ...
    'xDesign selected or inferred these workspace contexts for this turn. Treat absolute paths as reference context unless the user explicitly asks to edit them.',
...
export function ProjectView({
  ...
  // An xDesign Cloud run needs a wallet, and the ONLY client-side veto is
  ...
  const handleSend = useCallback(
    async (
      ...
      // xDesign Cloud pre-run balance gate: a definitively insufficient
  ...
  // "Share to xDesign" — kicks off the bundled `od-share-to-community`
...
function pluginWorkflowTitle(action: PluginFolderAgentAction): string {
  return action === 'publish' ? 'Publish repo' : 'xDesign PR';
}
...
function pluginWorkflowPlannedSteps(action: PluginFolderAgentAction): string[] {
  ...
  return [
    'Ensure the xDesign fork exists',
...
const DESIGN_RESULT_DELIVERY_FAILED_DETAIL =
  'The design result was generated, but xDesign could not save it to the project.';
