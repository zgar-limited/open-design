...
export function EntryShell({
  ...
  async function handlePluginLoopSubmit(payload: PluginLoopSubmit) {
    // xDesign Cloud pre-run balance gate: hard blocks (empty wallet or
   ...
   * / Workspace 设置, and the "sign in to xDesign Cloud" callout still in
  ...
  async function pollAmrLoginCompletion(): Promise<boolean> {
    ...
    while (!amrLoginPollCancelledRef.current) {
      ...
      if (outcome === 'signed-in') {
        ...
        // shape (still showing the "sign in to xDesign Cloud" callout)
