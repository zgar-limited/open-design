// Content fixtures for the "Use xDesign everywhere" guide modal.
//
...
export const GUIDE_SECTIONS: GuideSection[] = [
  {
    id: 'overview',
    tabLabel: 'Overview',
    heading: 'xDesign works wherever your agent works',
    intro:
      'xDesign is more than a window — it is a local privileged daemon ' +
    ...
    bullets: [
      ...
      'MCP server — wires xDesign as a Model Context Protocol server so any MCP-capable agent can list skills, run scenarios, and read artifacts.',
      'HTTP API — `http://127.0.0.1:7456/api/*` REST + SSE endpoints; the same surface the web UI uses.',
      'Skills — drop-in `SKILL.md` packs (Claude-compatible) that any agent already on your PATH can invoke without xDesign at all.',
    ...
    snippets: [
      ...
      },
      {
        ...
        body:
          ...
          '# Then start xDesign in the shell you normally use for dev:\n' +
          'pnpm tools-dev',
      },
    ],
    footer:
  ...
  },
  {
    id: 'cli',
    tabLabel: 'CLI · od',
    heading: 'Drive xDesign from any shell',
    intro:
  ...
  },
  {
    id: 'mcp',
    tabLabel: 'MCP server',
    heading: 'Expose xDesign as an MCP server to any coding agent',
    intro:
      'xDesign ships with a Model Context Protocol server (`od mcp`) ' +
      'that lets any MCP-capable client — Cursor, Claude Code, Antigravity, ' +
      'VS Code Copilot Chat, openclaw, hermes — discover xDesign tools ' +
    ...
    footer:
      'In the xDesign app, open Settings → Integrations to copy a ' +
  ...
  },
  {
    ...
    heading: 'Drop-in Skills for any agent — even without xDesign running',
    intro:
      'A Skill is a directory with a Claude-compatible `SKILL.md` ' +
      '(YAML front-matter + body). xDesign extends the format with the ' +
      '`od:` namespace (`mode`, `preview`, `design_system`, `inputs`, …) so ' +
      'the same artifact can be used both inside xDesign and by a vanilla ' +
    ...
    snippets: [
      {
        label: 'Minimal SKILL.md (Claude-compatible front matter + xDesign extras)',
        language: 'yaml',
        body:
          '---\n' +
      ...
      },
      {
        ...
        body:
          ...
          '# Shell 1: start xDesign after ingesting.\n' +
          'pnpm tools-dev\n' +
