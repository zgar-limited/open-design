...
- `apps/standalone`: Shell-neutral product composition for the Web + daemon closure. It exposes the application boundary using `packages/standalone-runtime` without owning Desktop IPC, windows, shell update UI, or artifact selection.
- `apps/packaged`: Thin packaged Electron runtime entry. It adapts packaged daemon/Web processes and paths to `apps/standalone`, registers the `od://` entry protocol, and delegates desktop host behavior to `apps/desktop`.
