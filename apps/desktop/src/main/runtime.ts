...
// sees the web's own "Loading xDesign…" shell flash between the splash and
...
function buildCrashReportUrl(ctx: RendererCrashScreenContext): string {
  ...
  const body = [
    "**What happened**",
    "The xDesign desktop window crashed several times in a row and showed the recovery screen.",
    "",
...
function buildCrashMailtoUrl(ctx: RendererCrashScreenContext): string {
  const subject = `xDesign keeps crashing (renderer ${ctx.reason})`;
  const body = [
    "The xDesign desktop app crashed several times in a row on my device.",
    "",
...
<html>
  ...
  <body>
    <div class="panel">
      <p class="title">xDesign keeps closing on this device</p>
      ...
      <p class="hint">If this keeps happening, quitting and reinstalling xDesign usually resolves it.</p>
    </div>
    <script>
      (function () {
...
export function createSplashWindow(): SplashWindowHandle {
  // xDesign ships light-only (the theme setting was removed), so pin the
...
export async function createDesktopRuntime(options: DesktopRuntimeOptions): Promise<DesktopRuntime> {
  ...
  const window = new BrowserWindow({
    ...
    // web's own "Loading xDesign…" shell.
    show: false,
    title: windowTitle,
    autoHideMenuBar: true,
    ...MAC_WINDOW_CHROME,
  ...
  const requireMainWindowSender = (event: Electron.IpcMainInvokeEvent): void => {
    if (event.sender !== window.webContents) {
      throw new Error("host IPC is only available to the main xDesign window");
    }
  };
  ...
  // reveal the web's own dark "Loading xDesign…" shell, and (b) the splash
